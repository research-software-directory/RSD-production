...
```bash
curl --location --output release.zip https://github.com/research-software-directory/RSD-as-a-service/releases/download/v1.0.1/deployment.zip && unzip release.zip
```
...
docker-compose exec --tty database pg_dump --format=tar --file=rsd-backup.tar --username=rsd --dbname=rsd-db && docker cp database:rsd-backup.tar rsd-backup.tar
