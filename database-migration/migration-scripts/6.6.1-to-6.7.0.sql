...
drop function if exists "public"."nassa_import"(slug character varying, brand_name character varying, description character varying, short_statement character varying, get_started_url character varying, repository_url character varying, license_value character varying, license_name character varying, license_url character varying, license_open_source boolean, related_modules character varying[], family_names_array character varying[], given_names_array character varying[], role_array character varying[], orcid_array character varying[], position_array integer[], categories jsonb, regular_mentions jsonb);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.software_import(community_slug character varying, slug character varying, brand_name character varying, concept_doi character varying, description character varying, description_url character varying, short_statement character varying, get_started_url character varying, repository_url character varying, code_platform platform_type, scraping_disabled_reason character varying, license_value character varying, license_name character varying, license_url character varying, license_open_source boolean, keywords_array citext[], related_modules character varying[], family_names_array character varying[], given_names_array character varying[], affiliation_array character varying[], role_array character varying[], orcid_array character varying[], position_array integer[], categories jsonb, regular_mentions jsonb)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
BEGIN
	...
	INSERT INTO software (
		...
		software_import.description_url,
		CASE WHEN software_import.description_url IS NULL THEN 'markdown'::description_type ELSE 'link'::description_type END,
